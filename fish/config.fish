...
function op
    set exit_after_opening 0
    for i in (seq (count $argv))
        if string match -q -- $argv[$i] "-d"
            if test (count $argv) -gt $i
                set exit_after_opening 1
                set --erase argv[$i]
                set flag $argv[$i]
                break
            else
                echo "Missing argument after -d flag"
                return
            end
        end
    end
    ...
    for arg in $argv
        if test -e $arg
            if test -d $arg
                # test if directory is '.' if so, open the current directory using ranger
                if test $arg = "."
                    nvim .
                else
                    cd $arg
                ...
                prime-run xdg-open $arg > /dev/null 2>&1 &
                disown
                if test $exit_after_opening -eq 0
                    sleep 0.5
                    exit 0
                end
            end
        else
            echo "File or directory $arg does not exist"
        end
    end
end
